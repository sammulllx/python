...
class firework(object):
    def __init__(self, color, speed, width, height):
        ...
        self.center = [random.randint(
            0, width-1), random.randint(0, height-1)]  # 烟花随机中心坐标
        ...
        for i in range(self.nParticle):
            ...
            # 椭圆参数方程
            x, y = self.ellipsePara[0] * \
                math.cos(theta*i+t), self.ellipsePara[1]*math.sin(theta*i+t)
            xx, yy = x*math.cos(self.rotTheta)-y*math.sin(self.rotTheta),  y * \
                math.cos(self.rotTheta)+x*math.sin(self.rotTheta)  # 平面旋转方程
    ...
    def extend(self):  # 粒子群状态变化函数线程
        for i in range(100):
            ...
            self.curParticle = [[one[0]*self.status/100, one[1]*self.status/100]
                                for one in self.oneParticle]  # 更新粒子群坐标
...
def colorChange(fire):
    ...
    # 当粒子寿命到70%时，颜色开始线性衰减
    def f(x, c): return hex(int(int(x, 16)*(100-c)/30))[2:]
    if cs > 70:
...
def appendFirework(n=1):  # 递归生成烟花对象
    if n > maxFireworks or len(Fireworks) > maxFireworks:
        ...
        cl = '#{0:0>6}'.format(hex(int(random.randint(0, 16777215)))[
                               2:])   # 产生一个0~16777215（0xFFFFFF）的随机数，作为随机颜色
        a = firework(cl, random.uniform(1.5, 3.5), width, height)
        # 建立粒子显示列表，‘particle’为一个烟花对象，‘points’为每一个粒子显示时的对象变量集
...
def show(c):
    ...
    for p in Fireworks:  # 根据每个烟花对象，计算其中每个粒子的显示对象
        ...
        if oneP.status == 100:  # 状态标识为100，说明烟花寿命结束
            ...
            li = [[int(cp[0]*2)+oneP.center[0], int(cp[1]*2)+oneP.center[1]]
                  for cp in oneP.curParticle]  # 把中心为原点的椭圆平移到随机圆心坐标上
            color = colorChange(oneP)  # 根据烟花当前状态计算当前颜色
            for pp in li:
                p['points'].append(c.create_oval(pp[0]-oneP.radius,  pp[1]-oneP.radius,
                                   pp[0]+oneP.radius,  pp[1]+oneP.radius,  fill=color))  # 绘制烟花每个粒子
